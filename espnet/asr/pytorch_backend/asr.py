...
def recog_embedding(args):
    ...
    print(args.result_label)
    ark_path = args.result_label.replace(".json",".ark")
    scp_path = args.result_label.replace(".json", ".scp")
    ...
    if args.batchsize == 0:
        with kaldiio.WriteHelper('ark,scp:{},{}'.format(ark_path, scp_path)) as writer:
            ...
            with torch.no_grad():
                for idx, name in enumerate(js.keys(), 1):
                    ...
                    enc_feats = model.encode(feat)
                    enc_feats = enc_feats.cpu().numpy()
                    logging.info(name)
                    writer(name, enc_feats)
