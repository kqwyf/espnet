        ...
        if layer_type == 'transformer':
            ...
            self.trans_layer = ConformerEncoderLayer(
                ...
                feed_forward_macaron=PositionwiseFeedForward(input_size, att_ff, dropout_rate=dropout,
                                                             activation=Swish()),
                conv_module=ConvolutionModule(input_size, kernel_size=5, activation=Swish()),
                dropout_rate=dropout,
                normalize_before=True,
            )
            pass
...
class LocalTransformer(nn.Module):
    ...
    def __init__(self, input_size, dropout=0, num_blocks=1, num_head=4, att_ff=1024, idx=1, layer_type="transformer",
                 downsample_local=False, block_size=100):
...
class GlobalTransformer(nn.Module):
    ...
    def __init__(self, input_size, dropout=0, num_blocks=1,
                 attention_dim=256, num_head=4, att_ff=1024, idx=1, layer_type="transformer", downsample_local=None, block_size=100):
        super(GlobalTransformer, self).__init__()

        self.downsample_local = downsample_local
        if self.downsample_local:
            self.down_sampling = nn.Conv1d(input_size, out_channels=input_size, kernel_size=15, stride=5, padding=7)
            self.down_sampling_activation = Swish()
            self.up_sampling = nn.ConvTranspose1d(input_size, out_channels=input_size, kernel_size=15, stride=5,
                                                  padding=7)
        self.input_size = input_size
        self.idx = idx
        if self.idx == 0:
    ...
    def forward(self, input):
        ...
        col_input = col_input.permute(0, 2, 3, 1).contiguous().view(batch_size * dim1, dim2, N)  # B*dim1, dim2, N
        if self.downsample_local:
            assert batch_size == 1, "bellowing operation may be risky when bs neq 1"
            col_input = col_input.permute(1, 2, 0)  # dim2, N,  B * dim1,
            col_input = self.down_sampling(col_input)  # dim2, N, B * dim1'
            col_input = self.down_sampling_activation(col_input)
            col_input = col_input.permute(2, 0, 1)  # B*dim1', dim2, N

        col_output = self.col_transformer(col_input)  # B*dim1, dim2, N
        if self.downsample_local:
            assert batch_size == 1, "bellowing operation may be risky when bs neq 1"
            col_output = col_output.permute(1, 2, 0)  # dim2, N, B * dim1*
            col_output = self.up_sampling(col_output, output_size=[dim1])  # dim2, N, B * dim1
            col_output = col_output.permute(2, 0, 1)
...
class GlobalTransformerV2(nn.Module):
    """
        Global Att
    """
    ...
    def __init__(self, input_size, dropout=0, num_blocks=1,
                 attention_dim=256, num_head=4, att_ff=1024, idx=1, layer_type="transformer", downsample_local=None, block_size=100):
        super(GlobalTransformerV2, self).__init__()
        self.block_size = block_size
        self.idx = idx
        if self.idx == 1:
            self.re_sampling = nn.Conv1d(input_size, out_channels=input_size, kernel_size=15, stride=5, padding=7)
            self.re_sampling_activation = Swish()
        elif self.idx == 4:
            self.re_sampling = nn.ConvTranspose1d(input_size, out_channels=input_size, kernel_size=15, stride=5,
                                                  padding=7)
            self.re_sampling_activation = Swish()
        else:
            self.re_sampling = None
    ...
    def do_re_sampling(self, input):
        batch_size, N, dim1, dim2 = input.shape
        input = input.permute(0, 3, 1, 2).contiguous().view(batch_size * dim2, N, dim1)  # B * dim2, N, dim1,
        input = self.re_sampling(input) if self.idx == 1 else self.re_sampling(input, output_size=[self.block_size])
        input = self.re_sampling_activation(input)  # B * dim2, N, dim1',
        input = input.view(batch_size, dim2, N, input.shape[-1])  # B, dim2, N, dim1',
        input = input.permute(0, 2, 3, 1)  # B, N, dim1', dim2
        return input
    ...
    def forward(self, input):
        ...
        if self.re_sampling:
            input = self.do_re_sampling(input)
        batch_size, N, dim1, dim2 = input.shape
        output = input
