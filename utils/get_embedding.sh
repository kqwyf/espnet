...
function download_models () {
    ...
    case "${models}" in
        ...
        "model.v1.tar.gz") share_url= ;;
        *) echo "No such models: ${models}"; exit 1 ;;
    esac

    if [ ! -e ${dir}/.complete ]; then
        tar xzvf model.v1.tar.gz -C ${dir} 
        touch ${dir}/.complete
    fi
}
...
if [ -z "${lang_model}" ] && ${use_lang_model}; then
    download_models
    lang_model=$(find ${download_dir}/${models} -name "rnnlm*" | head -n 1)
fi
